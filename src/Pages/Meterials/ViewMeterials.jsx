...
export default function ViewMeterials() {
    ...
    useEffect(() => {
        axios.post(
            import.meta.env.VITE_BASE_URL + import.meta.env.VITE_MATERIAL_VIEW,
            { name: nameFilter, page: currentPage }
        )
            .then((success) => {
                if (success.data._status === true) {
                    setMaterial(success.data._data);
                    setTotalPages(success.data._pagination.total_pages);
                } else {
    ...
    }, [apiStatus, nameFilter, currentPage]);   // <-- यहाँ totalPages हटाइएको छ
    ...
    const changeStatus = () => {
        ...
        if (checkedValue.length > 0) {
            axios.put(
                import.meta.env.VITE_BASE_URL + import.meta.env.VITE_MATERIAL_CHANGE_STATUS,
                { id: checkedValue }
            )
                .then((response) => {
    ...
    const deleteFile = () => {
        ...
        if (checkedValue.length > 0) {
            if (confirm('Are you Sure you Want to delete !!')) {
                axios.put(https, { id: checkedValue })
                    .then((response) => {
                        if (response.data._status == true) {
                            toast.success(response.data._message);
                            setApiStatus(!apiStatus);
                ...
                <div className="max-w-[1220px] mx-auto py-2">
                    ...
                    <div className="w-full min-h-[610px]">
                        <div className="max-w-[1220px] mx-auto py-5">
                            <div className='flex item-center flex-wrap sm:flex-nowrap gap-y-2 justify-between bg-slate-100 py-3 px-2 sm:px-4 rounded-t-md border border-slate-400'>
                                ...
                                <div className='flex justify-between '>
                                    ...
                                    <button
                                        ...
                                        className="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-3 sm:px-5  py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"> Change Status</button>
                                                    ...
                                                    <th scope="col" className="p-4">
                                                        <div className="flex items-center">
                                                            <input
                                                                ...
                                                                type="checkbox"
                                                                onChange={checkedAll}
                                                                checked={material.length > 0 && material.length === checkedValue.length}
                                                                    ...
                                                                    <td className="w-4 p-4">
                                                                        <div className="flex items-center">
                                                                            <input id="checkbox-table-search-1"
                                                                                ...
                                                                                className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                                                                            <label for="checkbox-table-search-1" className="sr-only" type="checkbox">checkbox</label>
                                                                        </div>
                                                                    </td>
