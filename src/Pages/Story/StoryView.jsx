...
export default function StoryView() {
    ...
    useEffect(() => {
        axios
            .post(
                `${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_WHY_CHOOSE_VIEW}`, { title: filterValue })
            .then((success) => {
                if (success.data._status === true) {
                    setWhyChooseUs(success.data._data)
    ...
    }, [apiStatus, filterValue]);


    // checkAll checkbox
    var checkedAll = () => {
        if (whyChooseUs.length !== checkedValue.length) {
            let data = whyChooseUs.map((value) => value._id);
    ...
    const changeStatus = () => {
        if (checkedValue.length > 0) {
            axios
                .put(
                    `${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_WHY_CHOOSE_CHANGE_STATUS}`,
                    { id: checkedValue }
                )
                .then((response) => {
    ...
    const deleteFile = () => {
        if (checkedValue.length > 0) {
            if (confirm('Are you sure you want to delete?')) {
                axios
                    .put(
                        `${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_WHY_CHOOSE_DELETE}`,
                        { id: checkedValue }
                    )
                    .then((response) => {
                        if (response.data._status === true) {
    ...
    return (
        <section className="w-full">
            ...
            <Breadcrumb path={"whyChooseUs"} link={'/why_choose_us/view'} path2={"View"} slash={"/"} />
                                            ...
                                            <th scope="col" className="p-4">
                                                <div className="flex items-center">
                                                    <input
                                                        ...
                                                        type="checkbox"
                                                        onChange={checkedAll}
                                                        checked={whyChooseUs.length > 0 && whyChooseUs.length === checkedValue.length}
