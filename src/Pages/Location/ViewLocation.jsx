...
export default function ViewLocation() {
  ...
  useEffect(() => {
    axios.post(import.meta.env.VITE_BASE_URL + import.meta.env.VITE_COUNTRY_VIEW, { name: nameFilter, page: currentPage })
      .then((success) => {
        if (success.data._status === true) {
          setCountry(success.data._data)
  ...
  // checkAll checkbox
  var checkedAll = () => {
    if (country.length != checkedValue.length) {
      var data = [];
  ...
  const changeStatus = () => {
    ...
    if (checkedValue.length > 0) {
      axios.put(
        import.meta.env.VITE_BASE_URL + import.meta.env.VITE_COUNTRY_CHANGE_STATUS,
        { id: checkedValue }
      )
        .then((response) => {
  ...
  const deleteFile = () => {
    ...
    if (checkedValue.length > 0) {
      if (confirm('Are you Sure you Want to delete !!')) {
        axios.put(import.meta.env.VITE_BASE_URL + import.meta.env.VITE_COUNTRY_DELETE, { id: checkedValue })
          .then((response) => {
            if (response.data._status == true) {
              toast.success(response.data._message);
              setApiStatus(!apiStatus);
                      ...
                      <th scope="col" className="p-4">
                        <div className="flex items-center">
                          <input
                            ...
                            type="checkbox"
                            onChange={checkedAll}
                            checked={country.length > 0 && country.length === checkedValue.length}
