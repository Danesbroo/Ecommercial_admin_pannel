...
export default function ViewSubCategory() {
  ...
  var checkedAll = () => {
    if (subCategory.length !== checkedValue.length) {
      let data = subCategory.map((value) => value._id);
  ...
  const changeStatus = () => {
    if (checkedValue.length > 0) {
      axios
        .put(
          `${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SUBCATEGORY_CHANGE_STATUS}`,
          { id: checkedValue }
        )
        .then((response) => {
  ...
  const deleteFile = () => {
    if (checkedValue.length > 0) {
      if (confirm('Are you sure you want to delete?')) {
        axios
          .put(
            `${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SUBCATEGORY_DELETE}`,
            { id: checkedValue }
          )
          .then((response) => {
            if (response.data._status === true) {
                      ...
                      <th scope="col" className="p-4">
                        <div className="flex items-center">
                          <input
                            ...
                            type="checkbox"
                            onChange={checkedAll}
                            checked={subCategory.length > 0 && subCategory.length === checkedValue.length}
