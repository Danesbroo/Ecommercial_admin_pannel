...
export default function SliderView() {
  ...
  useEffect(() => {
    axios
      .post(
        `${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SLIDER_VIEW}`, { name: nameFilter, page: currentPage})
      .then((success) => {
        if (success.data._status === true) {
          setSlider(success.data._data)
  ...
  }, [nameFilter, apiStatus]);

  var checkedAll = () => {
    if (slider.length !== checkedValue.length) {
      let data = slider.map((value) => value._id);
  ...
  const changeStatus = () => {
    if (checkedValue.length > 0) {
      axios
        .put(
          `${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SLIDER_CHANGE_STATUS}`,
          { id: checkedValue }
        )
        .then((response) => {
  ...
  const deleteFile = () => {
    if (checkedValue.length > 0) {
      if (confirm('Are you sure you want to delete?')) {
        axios
          .put(
            `${import.meta.env.VITE_BASE_URL}${import.meta.env.VITE_SLIDER_DELETE}`,
            { id: checkedValue }
          )
          .then((response) => {
            if (response.data._status === true) {
      ...
      <div className="w-full min-h-[610px]">
        <div className="max-w-[1220px] mx-auto py-5">
          <div className='flex item-center justify-between bg-slate-100 py-3 px-4 rounded-t-md border border-slate-400'>
            <h3 className="text-[18px] sm:text-[20px] md:text-[26px] font-semibold" >
              View Slider
            </h3>
            <div className='flex justify-between '>
              ...
              <button
                type="button"
                onClick={deleteFile}
                className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-3 sm:px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Delete </button>
            </div>
          </div>
                    ...
                    <tr>
                      <th scope="col" className="p-4">
                        <div className="flex items-center">
                          <input
                            onClick={checkedAll}
                            checked={slider.length > 0 && slider.length === checkedValue.length}
                          ...
                          return (
                            <tr key={i} className="bg-white  dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                              ...
                              <td className=" py-4">
                                {
                                  v.image !== '' ?
                                    <img className="w-10 h-10 rounded-full" src={imagePath + v.image} alt=" image" />
                                    :
                                    "N/A"
                                }
                              </td>
