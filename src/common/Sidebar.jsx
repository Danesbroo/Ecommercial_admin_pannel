...
export default function Sidebar({sidebarOpen, setSidebarOpen}) {
  ...
  return (
    ...
    <aside
      id="logo-sidebar"
      className={`w-64 h-screen bg-gray-50 transition-transform duration-300 z-50 fixed top-0 left-0
       ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 md:static
      `}
    >
      <div className="h-full overflow-y-auto px-3 py-4 bg-gray-50 dark:bg-gray-800 scrollbar-hide">
        <Link to={"/dashboard"}>
        <div className="flex justify-end"><ImCross onClick={()=> setSidebarOpen(false)} className="text-white md:hidden" /></div>
          ...
          <Link to={"/dashboard"}>
            <li className="pb-3">
              ...
              >
                ...
                <span onClick={()=> setSidebarOpen(false)} className="ms-3">Dashboard</span>
              </div>
            </li>
          </Link>
              ...
              <li key={index} className="cursor-pointer">
                ...
                <ul className={sidebar === id ? "block" : "hidden"}>
                  {subMenu.map((subItems, index) => {
                    return (
                      <Link to={subItems.link} key={index} onClick={() => setSidebarOpen(null)}>
                        <li>
